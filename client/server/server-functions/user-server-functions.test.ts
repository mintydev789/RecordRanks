...
//       }: {
//         body: { userId: string; permissions: Permissions };
...
//     const roles: OrganizationRole[] = ["mod"];
//     const user = (await db.query.users.findFirst({ where: { username: "user" } }))!;

//     const res = await updateMemberSF({ id: user.id, personId: user.personId, roles });
...
//       process.env.NEXT_PUBLIC_CONTACT_EMAILXXXXXXXX???,
...
//       const res = await updateMemberSF({ id: "INVALID", personId: 1, roles: ["user"] });
...
//         roles: emailNotVerifiedUser.role!.split(",") as OrganizationRole[],
//       });
...
//       const user = (await db.query.users.findFirst({ where: { username: "user" } }))!;
//       const res = await updateMemberSF({ id: user.id, roles: ["mod"] });
